...
export function ChatEntry(props: Readonly<{ value?: string; disabled?: boolean; attachments?: readonly ImageAttachmentRef[]; slashContext?: SlashCommandContext; onChange?: (value: string) => void; onPasteImage?: () => void; onRemoveLastAttachment?: () => void; onPaletteOpenChange?: (open: boolean) => void; onSubmit: (value: string) => boolean | void }>): React.JSX.Element {
  ...
  useInput((input, key) => {
    if ((key.ctrl || key.meta) && input === "v") { props.onPasteImage?.(); return; }
    ...
    if (key.return) {
      if (!valueRef.current.trim() && !props.attachments?.length) return;
    ...
    if (key.backspace || key.delete) { if (!valueRef.current && props.attachments?.length) props.onRemoveLastAttachment?.(); else replaceValue(valueRef.current.slice(0, -1)); return; }
    if (!key.ctrl && !key.meta && input) replaceValue(valueRef.current + input);
  });
  return <Box flexDirection="column" marginTop={1}>
    ...
    {props.attachments?.map((item, index) => <Text key={item.id} dimColor>[图片 {index + 1}：{sanitizeTerminalText(item.fileName)}]</Text>)}
    <Box flexDirection="column" borderStyle="round" paddingX={1} {...border()}>
    <Text {...accent()}>› {sanitizeTerminalText(value) || "请输入内容…"}</Text>
    <Text dimColor>{matches.length ? "↑/↓ 或 Tab 选择 · Enter 执行 · Esc 收起" : "Enter 发送 · Alt+Enter / Ctrl+J 换行 · Ctrl+V/Alt+V 图片"}</Text>
    </Box>
  </Box>;
}
