...
export async function runWebUi(options: Readonly<{ port?: number }> = {}): Promise<void> {
  const projects = new ActiveProjectController();
  await projects.openCurrentOrDefault();
  const client = new LocalUiCommandClient({ application: () => { const current = projects.current(); if (!current) throw new Error("WebUI Project is not open."); return current.application; }, projects: projects.projects, activateProject: async (projectId) => { await projects.activate(projectId); } });
