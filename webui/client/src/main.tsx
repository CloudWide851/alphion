...
function App(): React.JSX.Element {
  ...
  useEffect(() => { void fetch("/api/bootstrap", { method: "POST" }).then((response) => response.json()).then((value: { csrf: string }) => { setCsrf(value.csrf); setStatus("已连接"); }); }, []);
  useEffect(() => { if (!csrf) return; void reloadSessions(); }, [csrf, reloadSessions]);
  useEffect(() => {
    if (!csrf) return;
    ...
    for (const kind of ["run.delta", "run.finished", "agent.event", "stream.resync-required", "approval.challenge"]) source.addEventListener(kind, (event) => consumeEvent(JSON.parse((event as MessageEvent<string>).data) as UiEventEnvelope));
    return () => source.close();
  }, [csrf, reloadSessions]);

  const send = async () => {
    const content = draft.trim(); if (!content || !csrf) return;
    let session = active;
  ...
  const decideApproval = async (approved: boolean): Promise<void> => {
    if (!approval) return;
    try {
      const response = await fetch("/api/approval", { method: "POST", credentials: "same-origin", headers: { "content-type": "application/json", "x-alphion-csrf": csrf }, body: JSON.stringify({ requestId: approval.requestId, actionDigest: approval.actionDigest, ...(approval.shapeDigest ? { shapeDigest: approval.shapeDigest } : {}), approved }) });
  ...
  return <div className="app-shell">
    ...
    <main className="conversation">
      ...
      {settings ? <SettingsPanel api={api.execute} csrf={csrf} onProjectActivated={() => void reloadSessions()} /> : null}
...
function SettingsPanel({ api, csrf, onProjectActivated }: Readonly<{ api: (command: UiCommand) => Promise<UiCommandResult>; csrf: string; onProjectActivated: () => void }>): React.JSX.Element {
  const [diagnostic, setDiagnostic] = useState("选择一项查看");
  const [projects, setProjects] = useState<readonly { id: string; name: string }[]>([]);
  const loadProjects = () => void api({ kind: "project.list" }).then((result) => { const items = result.result as readonly { id: string; name: string }[]; setProjects(items); setDiagnostic(items.length ? "选择 Project 进行切换" : "尚未注册 Project"); });
  return <section className="settings-panel"><div className="settings-actions"><button onClick={loadProjects}>Projects</button><button onClick={() => void api({ kind: "provider.list" }).then((result) => setDiagnostic(JSON.stringify(result.result, null, 2)))}>Provider</button><button onClick={() => void api({ kind: "resource.list" }).then((result) => setDiagnostic(JSON.stringify(result.result, null, 2)))}>资源</button><button onClick={() => void api({ kind: "doctor" }).then((result) => setDiagnostic(JSON.stringify(result.result, null, 2)))}>doctor</button></div>{projects.length ? <div className="project-list">{projects.map((project) => <button key={project.id} onClick={() => void api({ kind: "project.activate", projectId: project.id }).then(() => { setProjects([]); setDiagnostic(`已切换至 ${project.name}`); onProjectActivated(); })}>{project.name}</button>)}</div> : null}<pre>{diagnostic}</pre><CredentialForm csrf={csrf} /><Info text="敏感凭据通过独立的一次性表单提交；无论成功或失败，输入都会立即清空，且不会写入浏览器存储。" /></section>;
}

function CredentialForm({ csrf }: Readonly<{ csrf: string }>): React.JSX.Element {
  ...
  const submit = async (event: React.FormEvent) => { event.preventDefault(); const profileId = profile.current?.value.trim() ?? ""; const value = secret.current?.value ?? ""; try { const response = await fetch(`/api/secret/provider/${encodeURIComponent(profileId)}`, { method: "POST", credentials: "same-origin", headers: { "content-type": "application/json", "x-alphion-csrf": csrf }, body: JSON.stringify({ secret: value }) }); setStatus(response.ok ? "凭据已导入" : "凭据导入失败"); } finally { if (profile.current) profile.current.value = ""; if (secret.current) secret.current.value = ""; } };
