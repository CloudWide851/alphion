...
export function AttachmentStrip(props: Readonly<{ attachments: readonly ImageAttachmentRef[]; client: SurfaceClient; removable?: boolean; onRemove?: (id: string) => void }>): React.JSX.Element | null {
  ...
  return <div className="attachment-strip" aria-label="图片附件">{props.attachments.map((ref, index) => <AttachmentThumb key={ref.id} refValue={ref} client={props.client} label={`图片 ${index + 1}`} {...(props.removable === undefined ? {} : { removable: props.removable })} {...(props.onRemove ? { onRemove: props.onRemove } : {})} />)}</div>;
}
function AttachmentThumb(props: Readonly<{ refValue: ImageAttachmentRef; client: SurfaceClient; label: string; removable?: boolean; onRemove?: (id: string) => void }>): React.JSX.Element {
  const [url, setUrl] = useState<string>(); const current = useRef<string | undefined>(undefined);
