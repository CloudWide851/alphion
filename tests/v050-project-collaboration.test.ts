...
test("SQLite v5 records domain identity and creates a recoverable v4 backup", async () => {
  ...
  try {
    ...
    database.exec("DROP TABLE collaboration_run_budgets; DROP TABLE collaboration_messages; DROP INDEX sessions_domain; ALTER TABLE sessions DROP COLUMN project_id; ALTER TABLE sessions DROP COLUMN domain_id; PRAGMA user_version = 4;");
    database.close();
